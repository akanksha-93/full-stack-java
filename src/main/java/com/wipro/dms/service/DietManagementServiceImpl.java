	...
	public boolean sendEmail(String email, String status, String comments) throws DMSException {
		...
		try {
			...
			if (status.equalsIgnoreCase("approved")) {
				...
				message = "Welocme to DMS program! /n Please refer below account details :" + "/n Userid - "
						+ user.getId() + "/n Password - " + user.getPassword() + "/n Referral Code - "
						+ user.getReferralCode();
			} else if (status.equalsIgnoreCase("rejected")) {
				message = "We regret to inform you that you can not be part of DMS program right now.But we would like to hear from you again, after rectification of below condition : "
						+ "/n" + comments;
			}
			msg.setTo(email);
